...
class DeterministicMDP(abc.ABC):
    ...
    If the observations are not the same as the state representation used for the successor function, then the
    obs_to_state method must be overridden.
    """
    ...
    def rollout_with_policy(self, policy, fixed_horizon=None, seed=None, render=False) -> TrajectoryWithRew:
        ...
        while not done or (fixed_horizon is not None and len(actions) < fixed_horizon):
            ...
            next_obs, reward, done, _ = self.step(self.get_action_index(action))
